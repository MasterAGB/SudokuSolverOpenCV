...
/// </summary>
public class WebCameraSudoku : MonoBehaviour
{
    /// <summary>
    /// Target surface to render WebCam stream
    /// </summary>
    public RawImage rawSurface;

    public bool resizeRawSurface = false;

    private Nullable<WebCamDevice> webCamDevice = null;
    public Mat webCamMat = null;
        ...
        set
        {
            ...
            else
            {
                throw new ArgumentException(
                    String.Format("{0}: provided DeviceName is not correct device identifier",
                        this.GetType().Name));
            }
        }
    }
    ...
    /// </summary>
    protected bool ProcessTexture(WebCamTexture input, ref Texture2D output)
    {
    ...
    void OnEnable()
    {
        rawSurface.gameObject.SetActive(true);
    }

    void OnDisable()
    {
        rawSurface.gameObject.SetActive(false);
    }
    ...
    private void RenderFrame()
    {
        ...
        {
            // apply
            rawSurface.texture = renderedTexture;

            if (resizeRawSurface)
            {
                // Adjust image ration according to the texture sizes 
                rawSurface.GetComponent<RectTransform>().sizeDelta =
